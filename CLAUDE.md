...
Three async tasks run concurrently via `tokio::select!` in main:
...
2. **HotkeyManager** (`src/hotkey/`) — Listens for a global hotkey via the `global-hotkey` crate. On trigger, spawns an async save task.
...
| `hotkey` | `src/hotkey/mod.rs`, `parser.rs` | Global hotkey registration and parsing (`"Ctrl+Shift+F9"` style strings) |
